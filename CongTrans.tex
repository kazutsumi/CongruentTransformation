...
\usepackage{amsthm,amsmath,amssymb, enumerate,bm,fancyhdr, braket, ascmac, graphicx, url}
...
\begin{proof}
  ...
  から，固有値 $\alpha$ の固有ベクトル $\bm{x} \left( \neq
    \bm{0}\right)$ に対して
  \[
    \bm{x} \cdot \bm{x} = T_A(\bm{x})\cdot T_A(\bm{x}) =
     (A\bm{x}) \cdot ( A\bm{x}) = (\alpha \bm{x}) \cdot (\alpha \bm{x}) = \alpha^2
    (\bm{x} \cdot \bm{x})
  \]
